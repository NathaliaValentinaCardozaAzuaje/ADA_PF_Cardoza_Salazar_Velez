...
int main(int argc, char* argv[]) {
    string rutaCSV = "data/WA_Fn-UseC_-Telco-Customer-Churn.csv";
    if (argc >= 2) {
        rutaCSV = argv[1];
    }
    ...
    vector<Solicitud> solicitudesParaGrafo = solicitudes;
    ...
    // Se construye desde el dataset original cargado.
    Grafo grafo = construirGrafo(solicitudesParaGrafo);
...
void guardarMST(
    ...
    for (const Arista& arista : mst) {
        archivo << arista.origen << " - "
                << arista.destino << " : "
                << arista.peso << "\n";
    }

    archivo << "\nPropiedad de eleccion codiciosa:\n";
    archivo << "Kruskal selecciona en cada paso la arista de menor peso que no forma ciclo. ";
    archivo << "Union-Find permite verificar si los dos nodos de una arista ya pertenecen ";
    archivo << "al mismo componente. Si ya estan conectados, agregar la arista produciria ";
    archivo << "un ciclo, por lo tanto se descarta. Si no estan conectados, la arista se ";
